...
**File:** `Healthcare_Patient_Analytics.xlsx`

A fully built analytics workbook with 6 sheets — no setup required, opens directly in Excel, Google Sheets, or LibreOffice:

| Sheet | What It Answers |
|---|---|
| **Executive Summary** | KPI cards — admissions, revenue, LOS, readmission rate, satisfaction, department & insurance summary tables |
| **Department Analysis** | Department-wise admissions and billing charts, top 10 diagnoses table |
| **Financial Analysis** | Revenue by insurance type (pie chart), monthly revenue trend (line chart) |
| **Clinical Outcomes** | Discharge status distribution (pie chart), readmission rate by department (bar chart), LAMA rate by department |
| **Patient Demographics** | Age group, gender, and blood group distribution charts, diagnosis-by-age breakdown |
| **Raw Data** | Full 5,000-row dataset with filters enabled for ad-hoc analysis |

> Open directly — no Power BI installation needed. Charts and tables are live and auto-filterable.
...
**Open the workbook:**
1. Open `Healthcare_Patient_Analytics.xlsx` in Excel, Google Sheets, or LibreOffice Calc
2. Navigate sheets via tabs: Executive Summary, Department Analysis, Financial Analysis, Clinical Outcomes, Patient Demographics, Raw Data
3. Use the auto-filter on the Raw Data sheet to slice by Hospital, Department, Insurance Type, or Date
